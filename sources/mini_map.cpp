...
void MiniMap::render(GraphicsPipeline& graphics,const DirectX::XMFLOAT2& player_pos, const DirectX::XMFLOAT2& player_forward,const DirectX::XMFLOAT2& camera_forward, std::vector<BaseEnemy*> enemy_list, std::vector<std::shared_ptr<BasePlayer>> player_list, int private_object_id)
{
	//レーダーあいこん
	...
	for (auto& p : player_list)
		{
			if (p->GetObjectId() == private_object_id) continue;
			const DirectX::XMFLOAT2 client_pos = { p->GetPosition().x,p->GetPosition().z };
			//プレイヤーから敵へのベクトル
			const DirectX::XMVECTOR Normal_P_To_CP_Vec = Math::calc_vector_AtoB_normalize(player_pos, client_pos);
			//プレイヤーの正面ベクトルと敵までのベクトルとの内積で間の角を出す
			DirectX::XMVECTOR Dot = DirectX::XMVector2Dot(P_Normal, Normal_P_To_CP_Vec);
			float dot = DirectX::XMVectorGetX(Dot);
			dot = acosf(dot);
			//正規化
			DirectX::XMFLOAT2 normal_p_to_e_vec{};
			DirectX::XMStoreFloat2(&normal_p_to_e_vec, Normal_P_To_CP_Vec);
			DirectX::XMFLOAT2 enemy_icon_pos;
			float cross{ (normal_p_to_e_vec.x * camera_forward.y) - (normal_p_to_e_vec.y * camera_forward.x) };
			//回転方向を指定
			dot = cross < 0 ? -dot : dot;

			DirectX::XMFLOAT2 client_player_icon_pos;


			const float length_p_to_e_vec = Math::calc_vector_AtoB_length(player_pos, client_pos);
			client_player_icon_pos.x = center_pos.x + length_p_to_e_vec / 2 * sinf(dot);
			client_player_icon_pos.y = center_pos.y - length_p_to_e_vec / 2 * cosf(dot);

			if (length_p_to_e_vec < 205.0f)
			{
				float angle = 0.0f;

				p->player_icon->begin(graphics.get_dc().Get());
				p->player_icon->render(graphics.get_dc().Get(), { client_player_icon_pos }, { 1.0f,1.0f },
					{ 16,16 }, { 1,1,1,1 }, { angle });
				p->player_icon->end(graphics.get_dc().Get());
			}
