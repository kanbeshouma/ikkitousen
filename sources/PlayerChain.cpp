			...
			if (search_time > 0)
			{
				...
				else
				{
					for (const auto& enemy : enemies)
					{
						...
						if (enemy->fComputeAndGetIntoCamera()) // 索敵時間内に一度でも視錐台に映ればロックオン(スタン関係なし)
						{
							chain_lockon_enemy_indexes.emplace_back(enemy->fGetObjectId()); // 登録
							LockOnSuggest enemy_suggest; // サジェスト登録
			...
			if (search_time > 0)
			{
				...
				else
				{
					for (const auto& enemy : enemies)
					{
						...
						if (enemy->fComputeAndGetIntoCamera()) // 索敵時間内に一度でも視錐台に映ればロックオン
						{
							chain_lockon_enemy_indexes.emplace_back(enemy->fGetObjectId()); // 登録
							LockOnSuggest enemy_suggest; // サジェスト登録
							enemy_suggest.position = enemy->fGetPosition();
